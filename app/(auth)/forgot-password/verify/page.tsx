...
import { forgotPasswordStart, forgotPasswordVerify } from "@/lib/api/auth";
...
export default function ForgotVerifyPage() {
  ...
  const submit = async (codeValue: string) => {
    ...
    setServerError(null);
    try {
      const { token } = await forgotPasswordVerify(email, codeValue);
      saveChallengeToken("reset", token);
  ...
  return (
    <AuthShell
      ...
      quote="Six digits is what stands between you and a fresh password."
      attribution="The archive"
      footer={
        <p className="text-center">
          Wrong email?{" "}
          <Link href="/forgot-password" className="font-medium text-moss-600 hover:underline">
            Start over
          </Link>
        </p>
      }
